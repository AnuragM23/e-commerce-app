...
function Header() {
  ...
  return (
    <header className='fixed top-0 left-0 m-auto max-padd_container w-full bg-white ring-1 ring-slate-900/5 z-10'>
        <div className='px-4 flexBetween py-3 max-xs:px-2'>
            ...
            {/* navbar Desktop */}
            <Navbar containerStyles={!menuOpened ?"hidden md:flex gap-x-5 xl:gap-x-10 medium-15": "hidden"} />
            ...
            <div className='flexBetween sm:gap-x-6 bold-16'>
              {!menuOpened?(<MdMenu className='md:hidden cursor-pointer hover:text-secondary mr-2 p-1 ring-1 ring-slate-900/30 h-8 w-rounded-full w-[40px] hover:ring-secondary' onClick={toggleMenu}/>):(<MdClose className='md:hidden cursor-pointer hover:text-secondary mr-2 p-1 ring-1 ring-slate-900/30 h-8 w-rounded-full w-[40px] hover:ring-secondary' onClick={toggleMenu}/>)}
              ...
              {/* <NavLink to={'logout'} className={"btn_secondary_rounded flexCenter gap-x-2 medium-14"}><img src={logout} alt='logoutIcon' height={19} width={19}/>Logout</NavLink> */}
              <NavLink to={'login'} className={"btn_secondary_rounded flexCenter gap-x-2 medium-14"}><img src={user} alt='userIcon' height={19} width={19}/>Login</NavLink>
            </div>
        </div>
    </header>
  )
