            ...
            <div>
              <FooterColumn title={FOOTER_CONTACT_INFO.title}>
                {FOOTER_CONTACT_INFO.links.map((link) => (
                  <Link to='/' key={link.lable} className='flex gap-4 md:flex-col lg:flex-row'>
                    <p>{link.label} : </p>
              ...
              <FooterColumn>
                <ul className='flex gap-4'>
                  {SOCIALS.links.map((link) => (
                    <Link to='/' key={link}><img src={link} height={22} width={22}  alt="socialIcon" /></Link>
                  ))}
                </ul>
              </FooterColumn>
            </div>
...
const FooterColumn = ({title, children}) =>{
  return(
    <div className='flex flex-col gap-5'>
      <h4 className='bold-18 whitespace-nowrap'>{title}</h4>{children}</div>
  );
}
